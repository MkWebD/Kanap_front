...
const emailRegex = new RegExp("^[a-zA-Z0-9-._]+[@]{1}[a-zA-Z0-9-_]+[.]{1}[a-zA-Z]{2,}$");
...
function createCartElement(localStored, apiData) {
		...
		constructor(type, text, append, className, attribute) {
			(this.type = type),
				...
				(this.createHtml = function () {
					...
					if (this.attribute) {
						if (Object.keys(this.attribute).includes("id") && Object.keys(this.attribute).includes("color")) {
	...
	for (let i = 0; i < localStored.length; i++) {
		...
		let apiElement = [...apiData].find((element) => element._id === localStored[i].id);
		...
		new HtmlElement("p", `${apiElement.price} €`, description.firstElementChild);
		...
		new HtmlElement("div", "", settings, "cart__item__content__settings__quantity");
		new HtmlElement("p", `Qté : ${localStored[i].quantity}`, settings.firstElementChild);
		...
		new HtmlElement("div", "", settings, "cart__item__content__settings__delete");
...
function calculateTotal(storage, apiData) {
	...
	for (let i = 0; i < storage.length; i++) {
		// Create variable that matches element in cart and element in global API
		let apiElement = [...apiData].find((element) => element._id === storage[i].id);
	...
	if (arrayPrice.length === 0) {
		...
		const priceTotal = arrayPrice.reduce((previousValue, currentValue) => previousValue + currentValue);
...
function modifyProducts() {
	...
	[...suppressButton].forEach((item) => {
		...
		item.addEventListener("click", () => {
			for (let i = 0; i < storage.length; i++) {
				// Check attributes
				if (storage[i].id === articleItemIdAttr && storage[i].color === articleItemColorAttr) {
					// Suppress item from DOM & storage
					storage = storage.filter((item) => {
						if (item.id == articleItemIdAttr && item.color == articleItemColorAttr) {
							return false;
						}
						return true;
					});
...
function checkForm() {
	class FormChecker {
		constructor(formField, regex, message) {
			(this.formField = formField), (this.regex = regex), (this.message = message);
		}
	}
	for (let i = 0; i < formFields.length; i++) {
		let formElement = new FormChecker(formFields[i], regexList[i], errorMessages[i]);
		let sibling = formElement.formField.nextElementSibling;
		formElement.formField.addEventListener("input", (e) => {
			if (e.target.value === null || e.target.value === "" || formElement.regex.test(e.target.value) === true) {
				sibling.innerHTML = "";
				return true;
			} else {
